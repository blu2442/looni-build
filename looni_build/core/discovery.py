"""On-disk discovery for looni-build tools.

A Python port of the ``_find_tool`` logic from ``looni-build.sh``, generalised
...
2. **Source tree** — the Python launcher is running from the repo checkout,
   alongside the ``looni-*_builder`` subdirectories. Tools resolve via
...
We additionally honour ``$LOONI_ROOT`` as an explicit override for the
source-tree root.
"""
...
from looni_build.core.tools import Tool, get_tool


# ── Repo root resolution ─────────────────────────────────────────────────────

# Markers that reliably identify a looni-build source checkout.
...
def resolve_root(start: Path | None = None) -> Path | None:
    """Locate the looni-build repo root.

    Search order:
      1. ``$LOONI_ROOT`` environment variable (if set and valid).
      2. *start* (defaults to CWD) and each of its ancestors.
      3. Directory containing the ``looni_build`` package — walk up from
         this file's location. Useful when installed editable from a clone.

    Returns ``None`` if no checkout is detectable.
    """
    env_root = os.environ.get("LOONI_ROOT")
    if env_root:
