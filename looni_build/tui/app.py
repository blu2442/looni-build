"""Main Textual application for the looni-build launcher.

Mirrors the look of ``looni-build.sh``'s menu (wolf banner + tool list) but
rendered as a modern TUI with keyboard navigation, grouping, and a footer of
keybinds. Tool selection hands off to :func:`looni_build.core.run_tool`,
...
from looni_build.core import TOOLS, find_tool, get_tool, run_tool
...
class LauncherApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(f":3  looni-build  •  Wine · Neutron · Proton  •  v{__version__}", id="subtitle")
        yield Static(
    ...
    def _launch_tool_suspended(self, key: str) -> None:
        ...
        with self.suspend():
            ...
            # matching the behaviour of ``looni-build.sh``.
            print("\033[2J\033[H", end="", flush=True)
            result = run_tool(key)
            if result.not_found:
                print(
                    f"\n  ✖  Could not find: {key}\n"
                    "  Make sure looni-build is installed (make install) "
